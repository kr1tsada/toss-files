...
export function FileRow({
  ...
  return (
    ...
    >
      <Icon
        size={15}
        className={file.is_dir ? "shrink-0 text-accent-text" : "shrink-0 text-fg-4"}
      />
      <span className="min-w-0 flex-1 truncate">{file.name}</span>
      <span className="w-28 shrink-0 text-right text-xs text-fg-5">
        {formatDate(file.modified)}
      </span>
      {!file.is_dir ? (
        <span className="w-16 shrink-0 text-right text-xs text-fg-5">
